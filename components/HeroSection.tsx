...
import Hero, { HERO_DEFAULT, RECO_PHOTOS, type HeroContent } from "./Hero";

type Stat = { value: number; label: string };
type Cand = { image_path: string; sort_order: number; recommended_on?: string | null };

export default async function HeroSection() {
  const [content, statsDoc, carouselDoc] = await Promise.all([
  ...
  return <Hero content={content} stats={statsDoc.items.slice(0, 4)} carousel={images} />;
}
